...
export default function Dashboard() {
  ...
  const openProject = (file) => {
    // Navigate or trigger download if needed
  };
      ...
      <div className="w-full max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 flex-1">
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-6 mb-10 sm:mb-16 border-b border-stone-200 pb-6 sm:pb-8">
          ...
          <div className="flex flex-wrap items-center gap-3 sm:gap-4 w-full sm:w-auto">
            ...
            <button onClick={() => navigate('/upload')} className="flex-1 sm:flex-none bg-stone-800 text-white px-4 sm:px-8 py-2.5 sm:py-3 rounded-xl sm:rounded-2xl text-sm sm:text-base font-semibold hover:bg-black hover:shadow-lg active:scale-95 transition-all duration-300">+ New Upload</button>
          </div>
        </div>
        ...
        {!Array.isArray(files) || files.length === 0 ? (
          <div className="w-full shadow-lg shadow-stone-200/50 border border-stone-200 bg-amber-50/30 p-8 sm:p-12 rounded-3xl sm:rounded-4xl max-w-2xl mx-auto text-center mt-8 sm:mt-12">
            ...
            <p className="text-sm sm:text-base text-gray-600 mb-6 sm:mb-8">You haven't uploaded any files yet.</p>
            <button onClick={() => navigate('/upload')} className="w-full sm:w-auto px-6 py-3 sm:py-3.5 text-sm sm:text-base font-medium tracking-wide text-white transition-all duration-300 bg-emerald-600 rounded-lg hover:bg-emerald-700 active:scale-95">Upload your first file</button>
          </div>
        ) : (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 sm:gap-6 lg:gap-8 pb-8">
            {files.map((file) => (
              <div key={file._id} onClick={() => openProject(file)} className="bg-white p-5 sm:p-6 lg:p-8 rounded-2xl sm:rounded-3xl shadow-sm border border-gray-100 hover:border-stone-300 hover:-translate-y-1 hover:shadow-xl active:shadow-md transition-all duration-300 cursor-pointer relative group flex flex-col h-full min-h-[200px] sm:min-h-[240px]">
                ...
                <div className="mt-4 sm:mt-6 flex items-center text-sm sm:text-base text-emerald-600 font-semibold sm:group-hover:translate-x-2 transition-transform duration-300">
                  <a href={file.cloud_storage_url} target="_blank" rel="noopener noreferrer" onClick={e => e.stopPropagation()}>Download File</a>
